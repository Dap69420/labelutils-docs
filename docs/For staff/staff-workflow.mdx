...
Pro and Pro+ teams can also review from the Vektra dashboard. Dashboard decisions update the database, DM the artist when approving or rejecting, and update the original staff card when the submission has Discord message references stored. See the [Dashboard guide](/dashboard) for the full web workflow.
...
Release requests are separate from demo submissions. They are for labels that want Vektra QC before Labelcaster delivery.

Server admins can submit a release request:

```text
/make-release
```

Admins can check the status of release requests submitted by the current server:

```text
/check-releases
```

Each release request stores its accepted or rejected status and decision reason. Accepted and rejected updates can also be sent to the submitter by DM from the owner review flow.

<Info>
  `/make-release` and `/check-releases` are admin-only commands. Dashboard release submission also requires server admin access.
</Info>

## DM reply forwarding

DM reply forwarding only works when the artist **replies directly** to the bot's DM message. Attachments are downloaded immediately and reuploaded into the staff thread so staff keeps a durable copy. If a file is too large for the server's upload limit, Vektra includes the original temporary Discord link as a fallback.
