...
<Info>
  Support tickets require their own private staff channel. This must be different from the demo submission staff channel set with `/setup_staff`.
</Info>
...
4. Vektra posts the ticket card in the private staff channel.
...
<Tip>
  Use a staff-only channel for `/ticket_channel` and a public or community-facing channel for `/ticket_panel`. Do not use the same staff channel that receives demo submissions.
</Tip>
...
<Note>
  Attachments in user replies are reuploaded into the matching staff thread when they fit Discord's upload limit. Oversized files fall back to temporary Discord links.
</Note>
